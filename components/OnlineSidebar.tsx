...
import { ClipboardList, Truck, Wallet, CalendarCheck, ArrowLeft, LogOut, ShoppingBag } from "lucide-react";
import { supabase } from "@/lib/supabase";
import ThemeToggle from "@/components/ThemeToggle";

const NAV = [
  { label: "Orders", href: "/online/orders", Icon: ClipboardList },
  { label: "Logistics", href: "/online/logistics", Icon: Truck, soon: true },
  { label: "Finance", href: "/online/finance", Icon: Wallet, soon: true },
  { label: "Attendance", href: "/online/attendance", Icon: CalendarCheck, soon: true },
];
    ...
    <aside className="hidden w-[248px] shrink-0 flex-col border-r border-line bg-surface dark:border-white/[0.06] dark:bg-[#201c17] md:flex">
        ...
        {NAV.map(({ label, href, Icon, soon }) => {
          ...
          return (
            <Link key={href} href={href}
              className={`flex items-center gap-3 rounded-xl2 px-3.5 py-2.5 text-[14px] transition ${on ? "bg-periwinkle-soft font-semibold text-ink dark:bg-white/[0.10] dark:text-white" : "text-muted hover:bg-panel hover:text-ink dark:text-[#a89f93] dark:hover:bg-white/[0.06] dark:hover:text-white"}`}>
      ...
      <div className="space-y-1 border-t border-line p-3 dark:border-white/[0.06]">
        <div className="flex items-center justify-between px-2 py-1">
          <span className="text-[11px] text-hint dark:text-[#8a8175]">Theme</span>
          <ThemeToggle />
        </div>
