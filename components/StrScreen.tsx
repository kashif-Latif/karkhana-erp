...
export default function StrScreen({ side: fixed }: { side: Side }) {
  ...
  /* Fixed by the route. Each department owns its own transfers, so this
     screen never offers the other one's stock. */
  const dir = fixed;
  ...
  return (
    <>
      <Topbar
        title={fixed === "factory" ? "STR — Factory" : "STR — Warehouse"}
        subtitle={fixed === "factory"
          ? "Packed goods leaving the factory for the warehouse"
          : "Goods leaving the warehouse for branches, malls and online"} />
      ...
      <div className="space-y-4 px-6 pb-12">
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
          ...
          <div className="rounded-card bg-amber-soft p-5">
            <p className="text-[12px] font-bold uppercase tracking-wide text-ink/55">
              {fixed === "factory" ? "Ready in factory" : "Held in warehouse"}</p>
            <p className="mt-1.5 text-[32px] font-extrabold leading-none tracking-tight text-ink">
              {n(fixed === "factory"
                ? ready.reduce((a, x) => a + Number(x.in_hand || 0), 0)
                : wItems.reduce((a, x) => a + Number(x.in_stock || 0), 0))}
            </p>
            <p className="mt-1 text-[12px] text-ink/60">
              {fixed === "factory" ? "packed, not yet transferred" : "available to send"}</p>
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {((fixed === "factory" && canFactory) || (fixed === "warehouse" && canWarehouse)) && (
            <button onClick={() => openForm(fixed)}
              ...
              <Plus size={15} /> {fixed === "factory" ? "Transfer to warehouse" : "Transfer out"}
            </button>
          )}
        </div>
