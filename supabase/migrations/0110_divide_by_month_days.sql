-- 0110_divide_by_month_days.sql
--
-- A FULL MONTH WORKED PAYS THE FULL SALARY, IN FEBRUARY AS IN AUGUST.
--
-- The divisor becomes the month's own length: ÷31 in August, ÷30 in September,
-- ÷28 in February. Turn up every day and you are paid your salary — the same
-- figure every month, with no 30-versus-31 distortion.
--
-- WHAT THE OLD FLAT ÷30 DID
--     31-day month   103.3% of salary
--     30-day month   100%
--     February        93.3%
--   and about 12.17 salaries a year instead of 12. Somebody who never missed a
--   day still earned less in February than in January, for no reason they could
--   see on their own payslip.
--
-- READ THE MID-MONTH FIGURE AS "EARNED SO FAR"
--   This is what looked wrong last time. On 29 August, 30 of 31 days have
--   happened, so the page shows about 97% of a month — not because anything is
--   missing, but because two days have not been worked yet. Under ÷30 the same
--   day showed 100%, which flattered the figure rather than describing it.
--   The number climbs each day and lands exactly on the salary at month end.
--
-- WORKING A DAY OFF STILL PAYS EXTRA. A full month plus a worked holiday is
-- 32/31 of a salary, and that is deliberate: the extra day is the reward for
-- working a day nobody was expected to.
--
-- This reverses 0102, which itself reversed 0100. Reinstated at Kashif's
-- instruction on 31 August after seeing both side by side.
--
-- Everything 0099 introduced stays: each day is still valued at the rate in
-- force on it, so a mid-month raise splits correctly.
--
...
as $function$
  with bounds as (
    select make_date(p_year, p_month, 1) as first_day,
           ...
           /* THE DIVISOR: how many days this month actually has. */
  ...
  tally as (
    select v.emp_id,
           ...
           sum(v.day_value * v.rate / (select month_days from bounds))          as gross,
           -- What the absent days would have paid, at their own day's rate.
           sum(case when v.status = 'A' then v.rate / (select month_days from bounds) else 0 end) as absent_deduction,
...
-- ===========================================================================
-- VERIFY — every August figure MOVES DOWN, because August has 31 days and was
-- being divided by 30. This is expected and is the whole point.
--
--   select name, counted_days, gross, advances, payable
--     from hub_monthly_payable(2026, 8) order by name;
--
-- A full August of 31 counted days now pays exactly the salary:
--   31/31 x 61,000 = 61,000 for Abdul Rehman.
-- With the 14 August holiday worked, 32/31 x 61,000 = 62,968 — his salary plus
-- one day, which is what working a holiday is meant to earn.
--
