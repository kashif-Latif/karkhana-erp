...
type Group = { id: string; name: string; units: { id: string; symbol: string }[] };
...
type BomLine = { group_id: string; quantity: string; unit_id: string };
...
export default function Articles() {
  ...
  const load = useCallback(async () => {
    if (!supabase) return;
    setLoading(true);
    const [a, g, pm] = await Promise.all([
      ...
      supabase.from("material_groups").select("id,name, group_units(units(id,symbol))").eq("is_active", true).order("name"),
    ...
    setGroups(((g.data as unknown as Record<string, unknown>[]) ?? []).map((r) => {
      ...
      return { id: r.id as string, name: r.name as string, units: gu.map((x) => x.units).filter(Boolean) as { id: string; symbol: string }[] };
    }));
  ...
  async function openRecipe(a: Article) {
    ...
    if (!supabase) return;
    const { data } = await supabase.from("article_bom").select("group_id,quantity,unit_id").eq("article_id", a.id);
    const existing = ((data as { group_id: string; quantity: number; unit_id: string }[]) ?? []).map((r) => ({ group_id: r.group_id, quantity: String(Number(r.quantity)), unit_id: r.unit_id }));
    setLines(existing.length ? existing : [{ group_id: "", quantity: "", unit_id: "" }]);
    setRecipeLoading(false);
  }
  const unitsFor = (gid: string) => groups.find((g) => g.id === gid)?.units ?? [];
  function setLine(i: number, patch: Partial<BomLine>) {
    setLines((ls) => ls.map((l, idx) => {
      if (idx !== i) return l;
      const next = { ...l, ...patch };
      if (patch.group_id) { const us = unitsFor(patch.group_id); next.unit_id = us.length === 1 ? us[0].id : ""; }
      return next;
    }));
  }
  const addLine = () => setLines((ls) => [...ls, { group_id: "", quantity: "", unit_id: "" }]);
  const removeLine = (i: number) => setLines((ls) => (ls.length === 1 ? ls : ls.filter((_, idx) => idx !== i)));

  async function saveRecipe() {
    setRecipeErr("");
    if (!supabase || !recipeFor) return;
    const valid = lines.filter((l) => l.group_id && l.unit_id && parseFloat(l.quantity) > 0);
    setSavingRecipe(true);
    const p_lines = valid.map((l) => ({ group_id: l.group_id, quantity: parseFloat(l.quantity), unit_id: l.unit_id }));
  ...
  return (
    <>
      <Topbar title="Articles" subtitle="Your garments & their material recipe" action={canManage ? undefined : undefined} />
          ...
          <>
            ...
            {articles.length === 0 ? (
              <div className="rounded-card bg-surface p-10 text-center shadow-card">
                ...
                <p className="mt-1 text-[13px] text-muted">Add your first garment (e.g. Kids Shirt) to get started.</p>
              </div>
            ) : (
                    ...
                    {articles.map((a) => (
                      <tr key={a.id} className="border-b border-line/60 last:border-0">
                        ...
                        <td className="px-5 py-3">
                          {a.bomCount > 0
                            ? <span className="rounded-full bg-success-soft px-2 py-0.5 text-[11.5px] font-semibold text-[#166534]">{a.bomCount} material{a.bomCount > 1 ? "s" : ""}</span>
                            : <span className="rounded-full bg-panel px-2 py-0.5 text-[11.5px] font-semibold text-muted">Not set</span>}
                        </td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-ink/30 p-4" onClick={() => !savingArt && setArtModal(null)}>
          <div className="w-full max-w-md rounded-card bg-surface p-6 shadow-card" onClick={(e) => e.stopPropagation()}>
            <div className="mb-3 flex items-center justify-between">
              <h2 className="text-[16px] font-extrabold">{artModal.id ? "Edit article" : "Add article"}</h2>
              <button onClick={() => setArtModal(null)} className="rounded-full p-1.5 text-muted hover:bg-panel"><X size={18} /></button>
            </div>
            ...
            <div className="mt-3 grid grid-cols-2 gap-3">
              <div>
                <label className="block text-[12px] font-medium text-muted">Garment type</label>
                <input value={gtype} onChange={(e) => setGtype(e.target.value)} placeholder="Shirt / Trouser" className={inp} list="gtypes" />
                <datalist id="gtypes"><option value="Shirt" /><option value="Trouser" /><option value="T-Shirt" /><option value="Pant" /></datalist>
              </div>
              <div>
                <label className="block text-[12px] font-medium text-muted">Audience</label>
                <select value={aud} onChange={(e) => setAud(e.target.value)} className={inp}>
                  <option value="">—</option>
            ...
            <div className="mt-3 grid grid-cols-2 gap-3">
              <div>
                <label className="block text-[12px] font-medium text-muted">Size (optional)</label>
                <input value={size} onChange={(e) => setSize(e.target.value)} placeholder="e.g. M" className={inp} />
              </div>
              {artModal.id && (
                <div>
                  <label className="block text-[12px] font-medium text-muted">Status</label>
                  <select value={active ? "1" : "0"} onChange={(e) => setActive(e.target.value === "1")} className={inp}>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-ink/30 p-4" onClick={() => !savingRecipe && setRecipeFor(null)}>
          <div className="max-h-[88vh] w-full max-w-lg overflow-y-auto rounded-card bg-surface p-6 shadow-card" onClick={(e) => e.stopPropagation()}>
            <div className="mb-1 flex items-center justify-between">
              <h2 className="text-[16px] font-extrabold">Recipe · {recipeFor.name}</h2>
              <button onClick={() => setRecipeFor(null)} className="rounded-full p-1.5 text-muted hover:bg-panel"><X size={18} /></button>
            </div>
            <p className="mb-4 text-[12.5px] text-muted">How much of each material <b>one piece</b> needs. Fabric by weight, zip by count.{!canManage && " (View only.)"}</p>

            {recipeLoading ? (
              <div className="flex items-center justify-center gap-2 py-8 text-muted"><Loader2 size={18} className="animate-spin" /> Loading…</div>
            ) : (
              <>
                <div className="mb-2 grid grid-cols-12 gap-2 px-1 text-[11px] uppercase tracking-wide text-muted">
                  <div className="col-span-6">Material</div><div className="col-span-3">Per piece</div><div className="col-span-2">Unit</div><div className="col-span-1"></div>
                </div>
                <div className="space-y-2">
                  {lines.map((l, i) => (
                    <div key={i} className="grid grid-cols-12 items-center gap-2">
                      <div className="col-span-6">
                        <select value={l.group_id} disabled={!canManage} onChange={(e) => setLine(i, { group_id: e.target.value })} className={inpSm}>
                          <option value="">Choose material…</option>
                          {groups.map((g) => <option key={g.id} value={g.id}>{g.name}</option>)}
                        </select>
                      </div>
                      <div className="col-span-3">
                        <input type="number" step="any" value={l.quantity} disabled={!canManage} onChange={(e) => setLine(i, { quantity: e.target.value })} placeholder="0" className={inpSm} />
                      </div>
                      <div className="col-span-2">
                        <select value={l.unit_id} disabled={!canManage || !l.group_id} onChange={(e) => setLine(i, { unit_id: e.target.value })} className={inpSm}>
                          <option value="">—</option>
...
const inpSm = "w-full rounded-xl2 border border-line bg-canvas px-2.5 py-2 text-[13px] outline-none placeholder:text-hint focus:border-salmon-strong/50";
