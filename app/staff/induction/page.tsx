...
export default function InductionPage() {
  ...
  const [side, setSide] = useState("factory");
  ...
  function openAdd() {
    setOpen(true); setEdit(null); setKilling(false); setName(""); setSide("factory"); setJoined(new Date().toISOString().slice(0, 10)); setPhone("");
  ...
  function openEdit(r: Staff) {
    ...
    setSide(r.code?.startsWith("WR-") ? "warehouse" : "factory");
  ...
  return (
    <>
      <Topbar title="New Staff Induction" subtitle="Add a person and give them an employee ID" />
      ...
      <Modal open={open} onClose={() => setOpen(false)} title={edit ? `Edit ${edit.name}` : "Add employee"}>
        ...
        <div className="mt-3 grid grid-cols-2 gap-3">
          ...
          <Field label="Side">
            {/* Decides the employee ID: FT- or WR-. A man's floor can change;
                his side rarely does. */}
            <select value={side} onChange={(e) => setSide(e.target.value)} disabled={!!edit} className={inp}>
